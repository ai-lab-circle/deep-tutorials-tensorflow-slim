...
input_dir = './data/1-nuclei/original_images/'
output_dir = './data/1-nuclei/images/'

postive_folder = 'postive_images'
...
def makeNegativeMask(io, patchDim):
    ...
    adjusted_img = imadjust(ior, 0.05, 0.5)
    #print(adjusted_img)
    #adjusted_img.astype(np.uint8)

    #select the dark regions to have a nuclei estimate
    _, bw = cv2.threshold(adjusted_img, 77, 255, cv2.THRESH_BINARY_INV)
    #bw =  adjusted_img < 0.3
    ...
    bw = morphology.remove_small_objects(labels, 200).astype(np.uint8)
    mask = np.zeros(((io.shape[0]+2), (io.shape[1]+2))).astype(np.uint8)
...
def make_patches():
    ...
    #kyy temparal  only one image test
    #print(patients)
    #temp = ['8865']
    #patients = temp
    ...
    for ci in range(len(patients)):
        ...
        for fi in range(len(files)):
            ...
            if not os.path.exists(outdir + postive_folder):
                os.makedirs(outdir + postive_folder)
            fnames_subs_pos = func_extraction_worker_w_rots(outdir + postive_folder, image_base, io, hwsize, r, c, npositive_sample_per_image, 1, 'p', [0, 90])
            npos = len(fnames_subs_pos)

            if not os.path.exists(outdir + negative_folder):
                os.makedirs(outdir +  negative_folder)

            (r, c) = makeNegativeMask_dilate_sub(io_mask, 3).nonzero()
            r = r.tolist()
            c = c.tolist()
            fnames_subs_neg1 = func_extraction_worker_w_rots(outdir + negative_folder, image_base, io, hwsize, r, c, npos*ratio_nuclei_dilation, 0, 'e', [0, 90])
            (r, c) = makeNegativeMask(io, 50).nonzero()
            r = r.tolist()
            c = c.tolist()
            fnames_subs_neg2 = func_extraction_worker_w_rots(outdir + negative_folder, image_base, io, hwsize, r, c, npos*ration_background, 0, 'b', [0, 90])
