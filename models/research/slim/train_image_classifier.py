...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    tf.logging.info('name : %s , split_name : %s , dir : %s' % (FLAGS.dataset_name, FLAGS.dataset_split_name, FLAGS.dataset_dir))
    ...
    #kimyy
    #config = tf.ConfigProto(device_count={'CPU':0}) # mask GPUs visible to the session so it falls back on CPU
    slim.learning.train(
        train_tensor,
